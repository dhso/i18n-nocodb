...
import type { LangCode } from './components/lang/lang'
...
type TranslateQueryBigBang = Omit<BaseTranslateQuery, 'mode' | 'action' | 'selectedWord' | 'detectFrom' | 'detectTo'> & {
  mode: 'big-bang'
...
export async function translate(query: TranslateQuery) {
  ...
  const toChinese = chineseLangCodes.includes(targetLangCode)
  ...
  switch (query.action.mode) {
    case null:
    case undefined:
      if ((query.action.rolePrompt ?? '').includes('${text}') || (query.action.commandPrompt ?? '').includes('${text}')) {
        contentPrompt = ''
      } else {
    ...
    case 'translate':
      ...
      commandPrompt = targetLangConfig.genCommandPrompt(sourceLangConfig, quoteProcessor.quoteStart, quoteProcessor.quoteEnd)
